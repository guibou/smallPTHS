/*
  Note: that's a copy of the "explicit.cpp" file found on
...
Vec
radiance (const Ray & r, int depth, unsigned short *Xi, int E = 1)
{
    ...
    {                // Ideal DIFFUSE reflection
        ...
        Vec w = nl, u =
            ((fabs (w.x) > .1 ? Vec (0, 1) : Vec (1)) % w).norm (), v = w % u;
        Vec d =
        ...
        for (int i = 0; i < numSpheres; i++)
        {
            ...
            Vec sw = s.p - x, su =
                ...
                sw % su;
            double cos_a_max =
                sqrt (1 - s.rad * s.rad / (x - s.p).dot (x - s.p));
    ...
    double nc = 1, nt = 1.5, nnt = into ? nc / nt : nt / nc, ddn =
        r.d.dot (nl), cos2t;
    ...
    double a = nt - nc, b = nt + nc, R0 = a * a / (b * b), c =
        1 - (into ? -ddn : tdir.dot (n));
    double Re = R0 + (1 - R0) * c * c * c * c * c, Tr = 1 - Re, P =
        .25 + .5 * Re, RP = Re / P, TP = Tr / (1 - P);
    return obj.e + f.mult (erand48 (Xi) < P ?    // Russian roulette
                                        radiance (reflRay, depth,
                                                  Xi) * RP : radiance (Ray (x,
                                                                            tdir),
                                                                       depth,
                                                                       Xi) *
                           TP);
...
#pragma omp parallel for schedule(dynamic, 1) private(r)    // OpenMP
        ...
        for (unsigned short x = 0, Xi[3] =
            ...
            for (int sy = 0, i = (h - y - 1) * w + x; sy < 2; sy++)    // 2x2 subpixel rows
                ...
                {            // 2x2 subpixel cols
                    ...
                    {
                        double r1 = 2 * erand48 (Xi), dx =
                            r1 < 1 ? sqrt (r1) - 1 : 1 - sqrt (2 - r1);
                        double r2 = 2 * erand48 (Xi), dy =
    ...
    FILE *f = fopen ("image_cpp.ppm", "w");    // Write image to PPM file.
    fprintf (f, "P3\n%d %d\n%d\n", w, h, 255);
    for (int i = 0; i < w * h; i++)
